...
param (
    ...
    #[System.Management.Automation.SwitchParameter]$NoConn
)
        ...
        if ($TargetObject) {
            ...
            Else {
                ...
                if (((Get-SourceMailboxStatistics $SourceIdentity).TotalDeletedItemSize -replace '^.+\((.+\))', '$1' -replace '\D' -as [int]) -gt ([int64]($TargetObject.RecoverableItemsQuota -replace '^.+\((.+\))', '$1' -replace '\D'))) {
                    #if (([int64]($SourceObject.RecoverableItemsQuota -replace '^.+\((.+\))','$1' -replace '\D')) -ne ([int64]($TargetObject.RecoverableItemsQuota -replace '^.+\((.+\))','$1' -replace '\D'))){
                    ...
                    #Write-Host ">> Error: RecoverableItemsQuota on SOURCE mailbox is not the same as TARGET MailUser RecoverableItemsQuota. This will cause the migration to fail" -ForegroundColor Red
                }
...
#Bypass connection to tenants prompt and use the existing sessions
